 ...
 * @return string
 */
/*if ( ! function_exists('bible_upload_url'))
{
    function bible_upload_url()
    {
        $CI =& get_instance();
        ...
        if ( ! $slug)
        {
            ...
            $Page = $CI->cache->model('entries_cache_model', 'cacheable_get_by', array('id' => $this->settings->content_module->site_homepage), 'entries');
        }
        else
        {
            // Query and cache a list of all page slugs
            $all_pages = $CI->cache->model('entries_cache_model', 'cacheable_page_slugs', array(), 'entries');

            // Find a page slug matching
            // the URI segments from cache/db
            if (isset($all_pages[$slug]))
            {
                $Page = $CI->cache->model('entries_cache_model', 'cacheable_get_by', array('id' => $all_pages[$slug]), 'entries');
            }
        }
       print_r($Page);
        die;  
