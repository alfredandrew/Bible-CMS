 ...
 * @author     Dan Horrigan
 * @license    
 * @  2011 - 2012 Dan Horrigan
 */

// CMS Golden Bible added exception class to prevent creating new file
class LexParsingException extends Exception { }

// CMS Golden Bible added Lex to class name to prvent using namespaces
// class Parser
class Lex_Parser
{
    // CMS Golden Bible allow php
    // protected $allowPhp = false;
    protected $allowPhp = true;
    protected $regexSetup = false;
    protected $scopeGlue = '.';
    protected $tagRegex = '';
    // CMS Golden Bible enable cumulative no parse
    ...
    public function parse($text, $data = array(), $callback = false, $allowPhp = false)
    {
        ...
        // CMS Golden Bible parse variables again so we can support variables inside of variables ex: {{ {{ lang }}_title }}
        $text = $this->parseVariables($text, $data, $callback);
        $text = $this->injectExtractions($text, 'callback_blocks');

        // CMS Golden Bible fix to extract no parse from $data
        $text = $this->extractNoparse($text); 
    ...
    public function parseCallbackTags($text, $data, $callback)
    {
        ...
        while (preg_match($regex, $text, $match, PREG_OFFSET_CAPTURE)) {
            ...
            // CMS Golden Bible pass data to callback function
            ...
            // CMS Golden Bible fix to extract noparse from callback return content
    ...
    public function parseConditionals($text, $data, $callback)
    {
        ...
        foreach ($matches as $match) {
            ...
            // CMS Golden Bible fix for variable conditionals with variable data that contain callbacks
            $tempCondition = $condition;
            $condition = preg_replace_callback('/\b('.$this->variableRegex.')\b/', array($this, 'processConditionVar'), $condition);

            // CMS Golden Bible fix for variable conditionals with data that contain callbacks
            // if ($callback) {
            ...
            // CMS Golden Bible don't need to parse variables again
    ...
    protected function setupRegex()
    {
        ...
        // CMS Golden Bible added variable {{ variable='whatever' }}
    ...
    protected function parsePhp($text)
    {
        ...
        if ($result === false) {
            ...
            // CMS Golden Bible adjustment to not use namespaces
